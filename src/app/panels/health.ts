  ...
  template: `
    <div class="card">
      ...
      <p class="hint">
        ...
        <code>has_main_web_app</code> (Mini App registered) and
        <code>can_join_groups</code> (group binding possible).
      </p>
      <button class="primary" (click)="getMe()" [disabled]="me.loading()">
        Run getMe
      </button>
    ...
    <div class="card">
      ...
      <p class="hint">
        Free health check. Alert if <code>pending_update_count</code> keeps
        growing (handler down/slow); read <code>last_error_message</code> +
        <code>last_error_date</code> for delivery failures.
      </p>
