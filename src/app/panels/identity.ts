...
@Component({
  ...
  template: `
    <div class="card">
      <h2>Language</h2>
      <p class="hint">
        Bot name & descriptions are per-language. Leave as <b>Default</b> for the fallback shown to
        everyone; pick a language for a dedicated variant (set the Vietnamese copy as Default, then
        add an <code>en</code>
        variant).
      </p>
      <select [(ngModel)]="lang">
    ...
    <div class="card">
      ...
      <div class="row" style="margin-top:10px">
        ...
        <button class="primary" (click)="setName()" [disabled]="nameSlot.loading()">
          Set name
        </button>
      </div>
    ...
    <div class="card">
      ...
      <textarea
        maxlength="512"
        [(ngModel)]="desc"
        placeholder="Get task notifications, run quick commands, open the Zira Mini App…"
      ></textarea>
      <div class="row" style="margin-top:10px">
        <button (click)="getDesc()" [disabled]="descSlot.loading()">Get</button>
        <button class="primary" (click)="setDesc()" [disabled]="descSlot.loading()">
          Set description
        </button>
      </div>
    ...
    <div class="card">
      <h2>
        Short description <span class="sub">(0–120 — profile + travels with shared links)</span>
      </h2>
      <textarea
        maxlength="120"
        [(ngModel)]="short"
        placeholder="Zira — quản lý công việc & dự án ngay trong Telegram."
      ></textarea>
      <div class="row" style="margin-top:10px">
        <button (click)="getShort()" [disabled]="shortSlot.loading()">Get</button>
        <button class="primary" (click)="setShort()" [disabled]="shortSlot.loading()">
    ...
    <div class="card">
      <h2>Profile photo</h2>
      <p class="hint">
        Static .JPG (or .PNG). Uploaded fresh each time (Telegram does not allow reusing a file_id
        for profile photos). Applies to all languages.
      </p>
      ...
      <div class="row" style="margin-top:10px">
        <button
          class="primary"
          (click)="setPhoto()"
          [disabled]="!photoFile() || photoSlot.loading()"
        >
          Upload photo
        </button>
...
export class IdentityPanel {
  ...
  async getDesc() {
    const r = await run(this.descSlot, () =>
      this.api.call<{ description: string }>('getMyDescription', this.langParam()),
    );
    if (r.ok && r.result) {
  ...
  async getShort() {
    const r = await run(this.shortSlot, () =>
      this.api.call<{ short_description: string }>('getMyShortDescription', this.langParam()),
    );
    if (r.ok && r.result) {
